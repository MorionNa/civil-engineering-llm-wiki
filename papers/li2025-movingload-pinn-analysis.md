...
> **⚠️ 非线性类型：** 本文的"非线性"来自**动力响应本身**（移动荷载激发的振动），**材料本构为线弹性**（Euler-Bernoulli 梁）。与 Raissi 2019 的算子非线性 ($u u_x$) 或 Goswami 2022 的材料非线性（损伤演化）不同。本文的物理约束项是线弹性梁的 PDE，非线性仅体现在动力求解过程中的数值行为。
