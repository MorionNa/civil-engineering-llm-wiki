...
updated: 2026-07-31
type: paper-analysis
tags: [neural-network, deep-learning, physics-simulation, scientific-machine-learning, ai4s, material-design, cross-domain-generalization]
...
第一性原理分子动力学能够提供高保真能量与原子力，但计算成本限制了可模拟的原子数和时间尺度。经验势计算快，却受固定函数形式限制；机器学习原子势试图在接近第一性原理精度的同时维持较低推理成本。

高精度训练标签本身也很昂贵。若模型需要成千上万乃至百万个第一性原理构型，构建训练集会成为主要瓶颈。因此，能够把已知空间对称性作为先验并显著减少标签需求的模型具有直接工程价值。

## 2. Research Gap

已有很多 GNN 原子势只在旋转不变标量上进行消息传递，例如以距离和角度作为输入。它们虽然能保证最终能量不随旋转变化，但网络内部没有显式保留方向性张量信息，仍需依赖数据间接学习几何变换规律。

另一些工作直接预测原子力，但若力不是总势能对坐标的梯度，就不能从结构上保证能量守恒，可能影响长时分子动力学的稳定性与物理可信度。
...
NequIP 的内部节点特征是 O(3) 不可约表示的直和，包含不同旋转阶数和奇偶性的标量、向量与高阶张量。卷积滤波器由可学习径向函数和球谐方向基组成，输入特征与滤波器通过满足对称选择规则的张量积组合。
...
- 水数据的学习曲线显示，等变模型不仅整体误差更低，随样本数增加的下降斜率也不同于纯标量不变模型。
...
- **最大旋转阶不是越高越好。** 更高 $l$ 增加表示能力和计算成本，其收益取决于系统、cutoff 和训练预算。
...
- 原论文采用的“减去训练集总能量均值”归一化在变尺寸体系中需谨慎，后续工作进一步强调了尺寸广延性问题。
...
| 消息传递层 | 与 MechConv 的矩阵边权和物理状态传播结合 |

## 11. 研究机会 (Research Opportunity)

1. 构造适合梁、柱、墙和节点局部坐标系的 SE(2)/SE(3) 等变 MechConv；
2. 将材料本构作为可替换局部模块，同时让几何等变骨干保持不变；
3. 用局部势能—内力一致性约束替代完全独立的恢复力回归；
4. 比较 NequIP 式多跳消息传递、Allegro 式严格局部高阶表示和普通 MechConv 的精度—扩展性前沿；
5. 借鉴 [[sevennet]] 的逐层特征/梯度通信实现子结构分布式训练；
6. 在全局动力平衡 $M\ddot x+C\dot x+f_{int}(x,z)=F$ 外增加局部等变与能量约束；
7. 研究支座、边界、重力方向和各向异性材料对“允许的对称群”的破坏，避免机械照搬完整 E(3) 对称性；
8. 用 [[kolmogorov-n-width-piml]] 搜索最难结构—地震动—本构任务，检验等变表示是否真正改善最坏情形泛化。
...
| 训练 | V100、float32、Adam AMSGrad、EMA、按验证力损失调度 |
| 复现风险 | 任务相关超参数多；旧版本软件与当前生态存在差异；部分数据依赖外部数据库 |
