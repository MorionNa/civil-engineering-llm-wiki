...
> **⚠️ 非线性类型：** 本文的物理模型为**线弹性振动**（Euler-Bernoulli 梁、弹性板），**材料本构为线性**。"非线性"仅指 PINN 训练过程本身的非线性优化行为（非凸损失、硬约束引入的数值刚度），不是 PDE 的非线性项。与 Raissi 的 $u u_x$ 算子非线性或 Goswami 的损伤材料非线性不同。

结构振动分析是航空航天、土木工程、机械设计中的核心问题
